...
try:
    with open("Oliver_Stewart-Resume.pdf", "rb") as file:
        response = requests.post(
...
while True:
    try:
        ...
        print(f"\r{' ' * shutil.get_terminal_size().columns}\rStatus code: {response.status_code} Response: {response.text} Elapsed: {str(timedelta(seconds=int(time.time() - start_time)))}", end="")
...
print("Finished indexing job:", uuid)